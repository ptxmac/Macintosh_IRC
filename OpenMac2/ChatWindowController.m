...
{
    // the file has been reformate, use a different url if you want old versoins to still understand the file ;)
    NSString *theContents = [NSString stringWithContentsOfURL:[NSURL URLWithString:[VERSION_URL stringByAppendingString:[NSString stringWithFormat:@"?version=%i", APPBUILDNUMBER]]]];

    NSArray *separateSemiColon;
    ...
    // no need to edit below this line in this function :)

    if (!theContents)
    {
        [self appendString:[NSString stringWithFormat:loc(@"version checker was unable to connect"),APP_URL] kind:@"TextColor"];
        return;
    }

    separateSemiColon = [theContents componentsSeparatedByString:@";"];
    if ([separateSemiColon count] == 4)
    {
        versionString = [separateSemiColon objectAtIndex:0];
        buildNumber = [[separateSemiColon objectAtIndex:1] intValue];
        dateString = [separateSemiColon objectAtIndex:2];
        urlString = [separateSemiColon objectAtIndex:3];
        featureString = @"";
    }
    else	if ([separateSemiColon count] == 5)
    {
        versionString = [separateSemiColon objectAtIndex:0];
        buildNumber = [[separateSemiColon objectAtIndex:1] intValue];
        dateString = [separateSemiColon objectAtIndex:2];
        urlString = [separateSemiColon objectAtIndex:3];
        featureString = [separateSemiColon objectAtIndex:4];
    }
    else
    {
        [self appendString:[NSString stringWithFormat:loc(@"error 2 while checking the current version"),THIS_CHAT_APP, APP_URL] kind:@"TextColor"];
        return;
    }
    if (!(versionString && buildNumber && dateString && urlString && featureString))
    {
        [self appendString:[NSString stringWithFormat:loc(@"error 3 while checking the current version"),THIS_CHAT_APP, APP_URL] kind:@"TextColor"];
        return;
    }
    if (buildNumber > APPBUILDNUMBER)
    {
        [self appendString:[NSString stringWithFormat:loc(@"new version available"), THIS_CHAT_APP, versionString, dateString, urlString, featureString] kind:@"TextColor"];
    }
    else
    {
        [self appendString:[NSString stringWithFormat:loc(@"You already have the latest version"),THIS_CHAT_APP] kind:@"TextColor"];
    }
}
...
- (void)launchCheckVersion
{
    ...
    // the file has been reformate, use a different url if you want old versoins to still understand the file ;)
    NSString *theContents = [NSString stringWithContentsOfURL:[NSURL URLWithString:[VERSION_URL stringByAppendingString:[NSString stringWithFormat:@"?launchVersion=%i", APPBUILDNUMBER]]]];
    ...
    if (!theContents)
    {
        //[self appendString:[NSString stringWithFormat:@"The version checker was unable to connect to the version server. Please make sure that your internet connection is active and try again. If all else fails, check %@ to see if there is a new version available. (Error 1: Connection Failed) \r\n",APP_URL] kind:@"TextColor"];
        return;
    }
    ...
    {
        //[self appendString:[NSString stringWithFormat:@"There was an error while checking the current version. Please download the latest version of %@ by visiting %@ . (Error 2: version file mal-formated)\r\n",THIS_CHAT_APP, APP_URL] kind:@"TextColor"];
        return;
    }
    if (!(versionString && buildNumber && dateString && urlString && featureString))
    {
        //[self appendString:[NSString stringWithFormat:@"There was an error while checking the current version. Please download the latest version of %@ by visiting %@ . (Error 3: version file mal-formatted)\r\n",THIS_CHAT_APP, APP_URL] kind:@"TextColor"];
        return;
    }
    if (buildNumber > APPBUILDNUMBER)
    {
